...
# Using simple arrays instead of associative arrays for compatibility with older bash
...
update_directory_build_props() {
    local new_version="$1"
    local props_file="$(dirname "$0")/Directory.Build.props"
...
for i in "${!PLATFORM_RIDS[@]}"; do
    ...
    output_dir="$(dirname "$0")/src/Native/Baml.Net.Native.$rid/runtimes/$rid/native"
